...
st.title("OpenCoverage")
st.caption("UAV coverage path planning — upload a survey polygon and plan a mission.")
...
st.markdown(
    ...
    in the **sidebar** (arrow `>` top-left if it is hidden), then click **Plan mission**.
    """
)
        ...
        for index, mission in enumerate(missions, start=1):
            ...
            with st.expander(segment_label, expanded=len(missions) == 1):
                ...
                with col_stats:
                    ...
                    download_name = (
                        f"mission_{index}.waypoints" if len(missions) > 1 else "mission.waypoints"
                    )
                    st.download_button(
